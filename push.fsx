...
let echoPS (mailbox:Actor<_>) = 
    let mutable count=0
    let mutable s=rnd //Get actor number
    let mutable a=0
    let mutable gee=0
    let mutable w=1
    ...
    let rec loop () = actor {
        ...
        |Neigbhour (s_send, w_send, num) ->
            ...
            if(ratioList.Count>=3 && (ratioList[ratioList.Count.Count-1]-float(s/w))>(10**(-10)) && (ratioList[ratioList.Count-2]-float(s/w))>(10**(-10)) && (ratioList[ratioList.Count-3]-float(s/w))>(10**(-10))) then //Stopping Condition
                //count<-count+1
                //if(count<>3) then
...
let fullTopology(n: int)=
    ...
    echoActorsPS.[rnd] <! Self(1,rnd,rnd)
